...
export function ReviewsSection({ professionalProfileId }: ReviewsSectionProps) {
  ...
  const { getReviews, createReview, updateReview, deleteReview, getAverageRating, loading } = useReviews();
  ...
  const handleSubmit = async () => {
    ...
    if (editingReview) {
      const { success } = await updateReview(editingReview.id, formData.rating, formData.comment);
      if (success) {
  ...
  const userHasReviewed = reviews.some(review => review.reviewer_user_id === user?.id);
        ...
        {reviews.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-muted-foreground">
              Aún no hay reseñas para este profesional. ¡Sé el primero en dejar una!
            </p>
          </div>
        ) : (
