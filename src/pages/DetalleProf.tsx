...
export default function DetalleProf() {
  ...
  useEffect(() => {
    console.log('DetalleProf useEffect triggered with id:', id);
    if (id) {
      loadProfessional();
    }
  ...
  const handleContact = () => {
    ...
    if (professional?.whatsapp_phone) {
      ...
      const message = encodeURIComponent(`Hola ${professional.profile_full_name}, me interesa contactarte por tus servicios de ${professional.trade_name}.`);
      ...
      toast({
        ...
        description: "El profesional tiene información de contacto disponible para usuarios registrados",
      });
    } else {
      toast({
        title: "Contacto",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 border-t border-border">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Description */}
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-6">Sobre mi trabajo</h2>
            ...
            {professional.profile_skills && professional.profile_skills.length > 0 && (
              <div>
                <h2 className="text-2xl font-bold text-foreground mb-6">Especialidades</h2>
                <Card className="border-border/50">
                  <CardContent className="p-6">
                    <div className="flex flex-wrap gap-3">
                      {professional.profile_skills.map((skill: string, index: number) => (
                        <Badge 
                          key={index} 
                          variant="secondary" 
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="bg-card border-border sticky top-6">
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  />
                  <div>
                    <p className="font-medium text-foreground text-sm">Contactar a {professional.profile_full_name.split(' ')[0]}</p>
                    <p className="text-xs text-muted-foreground">Responde en promedio en 2 horas</p>
                  </div>
                </div>
                ...
                {professional.hourly_rate && (
                  <div className="text-center py-3 border-y border-border">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-2xl font-bold text-foreground">ARS ${professional.hourly_rate}</span>
                      <span className="text-sm text-muted-foreground">/hora</span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">Tarifa orientativa</p>
                  </div>
                )}
            ...
            <Card className="border-border/50">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                  ...
                  <span className="text-sm font-medium">{professional.years_experience} años</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                ...
                {professional.profile_location_city && (
                  <div className="flex items-center justify-between py-2">
                    ...
                    <span className="text-sm font-medium">{professional.profile_location_city}</span>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-border/50 bg-muted/30">
              <CardContent className="p-6 space-y-3">
                <h3 className="font-medium text-sm">Tu seguridad es importante</h3>
          ...
          <Card className="bg-gradient-to-r from-primary/5 to-primary/10 border-primary/20">
            <CardContent className="p-8 text-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-2xl mx-auto">
                Contacta a {professional.profile_full_name} para recibir un presupuesto personalizado y comenzar a trabajar en tu proyecto hoy mismo.
              </p>
